...
export class API extends EventEmitter {
  ...
  validateKeyDerivation(opts, cb) {
    ...
    var testHardcodedKeys = () => {
      ...
      var xpub = Ltzcore.HDPublicKey.fromString(
    ...
    var testLiveKeys = () => {
      ...
      if (!xpriv) {
        xpriv = new Ltzcore.HDPrivateKey(c.xPrivKey);
      }
      xpriv = xpriv.deriveChild(c.getBaseAddressDerivationPath());
      var xpub = new Ltzcore.HDPublicKey(c.xPubKey);
  ...
  decryptBIP38PrivateKey(encryptedPrivateKeyBase58, passphrase, opts, cb) {
    ...
    var privateKey = new Ltzcore.PrivateKey(privateKeyWif);
    var address = privateKey.publicKey.toAddress().toString();
    var addrBuff = Buffer.from(address, 'ascii');
    var actualChecksum = Ltzcore.crypto.Hash.sha256sha256(addrBuff)
      .toString('hex')
      .substring(0, 8);
    var expectedChecksum = Ltzcore.encoding.Base58Check.decode(
  ...
  static _buildSecret(walletId, walletPrivKey, coin, network) {
    if (_.isString(walletPrivKey)) {
      walletPrivKey = Ltzcore.PrivateKey.fromString(walletPrivKey);
    }
    var widHex = Buffer.from(walletId.replace(/-/g, ''), 'hex');
    var widBase58 = new Ltzcore.encoding.Base58(widHex).toString();
    return (
  ...
  static parseSecret(secret) {
    ...
    try {
      ...
      var widHex = Ltzcore.encoding.Base58.decode(widBase58).toString('hex');
      var walletId = split(widHex, [8, 12, 16, 20]).join('-');

      var walletPrivKey = Ltzcore.PrivateKey.fromString(secretSplit[1]);
  ...
  _addSignaturesToLtzcoreTxBitcoin(txp, t, signatures, xpub) {
    $.checkState(
      txp.coin,
      'Failed state: txp.coin undefined at _addSignaturesToLtzcoreTxBitcoin'
    );
    $.checkState(
      txp.signingMethod,
      'Failed state: txp.signingMethod undefined at _addSignaturesToLtzcoreTxBitcoin'
    );
    const ltzcore = Ltzcore_[txp.coin];
    if (signatures.length != txp.inputs.length)
      throw new Error('Number of signatures does not match number of inputs');

    let i = 0;
    const x = new ltzcore.HDPublicKey(xpub);

    _.each(signatures, signatureHex => {
      try {
        const signature = ltzcore.crypto.Signature.fromString(signatureHex);
        ...
        const s = {
          ...
          sigtype:
            ...
            ltzcore.crypto.Signature.SIGHASH_ALL |
            ltzcore.crypto.Signature.SIGHASH_FORKID,
          publicKey: pub
        };
  ...
  _applyAllSignatures(txp, t) {
    ...
    _.each(sigs, x => {
      this._addSignaturesToLtzcoreTx(txp, t, x.signatures, x.xpub);
    });
  }
  ...
  createWallet(walletName, copayerName, m, n, opts, cb) {
    ...
    var walletPrivKey = opts.walletPrivKey || new Ltzcore.PrivateKey();
    ...
    var args = {
      ...
      m,
      n,
      pubKey: new Ltzcore.PrivateKey(walletPrivKey).toPublicKey().toString(),
      coin,
      network,
  ...
  recreateWallet(cb) {
    ...
    this.getStatus(
      ...
      err => {
        ...
        var c = this.credentials;
        var walletPrivKey = Ltzcore.PrivateKey.fromString(c.walletPrivKey);
  ...
  addAccess(opts, cb) {
    ...
    opts = opts || {};
    var requestPubKey = new Ltzcore.PrivateKey(opts.requestPrivKey)
      .toPublicKey()
      .toString();
  ...
  static upgradeMultipleCredentialsV1(oldCredentials) {
    ...
    if (newKeys.length > 0) {
      ...
      let credGroups = _.groupBy(newCrededentials, x => {
        ...
        let xpub = new Ltzcore.HDPublicKey(x.xPubKey);
