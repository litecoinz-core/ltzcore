...
export class PayPro {
  ...
  static _verify(requestUrl, headers, network, trustedKeys, callback) {
    ...
    let s_rBN = Ltzcore.crypto.BN.fromBuffer(s_r);
    let s_sBN = Ltzcore.crypto.BN.fromBuffer(s_s);

    let pub = Ltzcore.PublicKey.fromString(keyData.publicKey);

    let sig = new Ltzcore.crypto.Signature();
    sig.set({ r: s_rBN, s: s_sBN });

    let valid = Ltzcore.crypto.ECDSA.verify(hashbuf, sig, pub);
  ...
  static runRequest(opts, cb) {
    ...
    r.end((err, res) => {
      ...
      let hash = Ltzcore.crypto.Hash.sha256(Buffer.from(body, 'utf8')).toString(
        'hex'
      );
  ...
  static get(opts, cb) {
    ...
    PayPro.runRequest(opts, function (err, data) {
      ...
      try {
        ret.toAddress = new ltzcore.Address(data.outputs[0].address).toString(
          true
        );
      } catch (e) {
