...
export class Credentials {
  ...
  static fromDerivedKey(opts) {
    ...
    const priv = Ltzcore.PrivateKey(x.requestPrivKey);
    x.requestPubKey = priv.toPublicKey().toString();

    const prefix = 'personalKey';
    const entropySource = Ltzcore.crypto.Hash.sha256(priv.toBuffer()).toString(
      'hex'
    );
    const b = Buffer.from(entropySource, 'hex');
    const b2 = Ltzcore.crypto.Hash.sha256hmac(b, Buffer.from(prefix));
