...
export class Utils {
  ...
  static hashMessage(text) {
    ...
    ret = new Ltzcore.encoding.BufferReader(ret).readReverse();
    return ret;
  }
  ...
  static privateKeyToAESKey(privKey) {
    ...
    $.checkArgument(
      Ltzcore.PrivateKey.isValid(privKey),
      'The private key received is invalid'
    );
    var pk = Ltzcore.PrivateKey.fromString(privKey);
    return Ltzcore.crypto.Hash.sha256(pk.toBuffer())
      .slice(0, 16)
      .toString('base64');
  }
  ...
  static deriveAddress(scriptType, publicKeyRing, path, m, network, coin) {
    ...
    var ltzcore = Ltzcore_[chain];
    var publicKeys = _.map(publicKeyRing, item => {
      var xpub = new ltzcore.HDPublicKey(item.xPubKey);
    ...
    switch (scriptType) {
      case Constants.SCRIPT_TYPES.P2WSH:
        ...
        ltzcoreAddress = ltzcore.Address.createMultisig(
          publicKeys,
          m,
          network,
          nestedWitness,
      ...
      case Constants.SCRIPT_TYPES.P2SH:
        ltzcoreAddress = ltzcore.Address.createMultisig(publicKeys, m, network);
        break;
      case Constants.SCRIPT_TYPES.P2WPKH:
        ltzcoreAddress = ltzcore.Address.fromPublicKey(
          publicKeys[0],
          network,
          'witnesspubkeyhash'
        );
      ...
      case Constants.SCRIPT_TYPES.P2PKH:
        ...
        if (Constants.UTXO_COINS.includes(coin)) {
          ...
          ltzcoreAddress = Deriver.deriveAddress(
            chain.toUpperCase(),
            network,
            xPubKey,
            addressIndex,
  ...
  static signRequestPubKey(requestPubKey, xPrivKey) {
    var priv = new Ltzcore.HDPrivateKey(xPrivKey).deriveChild(
  ...
  static verifyRequestPubKey(requestPubKey, signature, xPubKey) {
    var pub = new Ltzcore.HDPublicKey(xPubKey).deriveChild(
      Constants.PATHS.REQUEST_KEY_AUTH
    ).publicKey;
