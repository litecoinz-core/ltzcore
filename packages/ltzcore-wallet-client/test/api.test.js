  ...
  var utxos = _.map(amounts, (amount, i) => {
    ...
    switch (scriptType) {
      ...
      case Constants.SCRIPT_TYPES.P2SH:
        scriptPubKey = new Ltzcore.Script.buildMultisigOut(address.publicKeys, requiredSignatures).toScriptHashOut();
        break;
      case Constants.SCRIPT_TYPES.P2WPKH:
      case Constants.SCRIPT_TYPES.P2PKH:
        scriptPubKey = new Ltzcore.Script.buildPublicKeyHashOut(address.address);
        break;
    }
    should.exist(scriptPubKey);

    var obj = {
      txid: new Ltzcore.crypto.Hash.sha256(Buffer.alloc(i)).toString('hex'),
      vout: 100,
...
blockchainExplorerMock.setUtxo = (address, amount, m, confirmations) => {
  ...
  blockchainExplorerMock.utxos.push({
    txid: new Ltzcore.crypto.Hash.sha256(Buffer.alloc(Math.random() * 100000)).toString('hex'),
    outputIndex: 0,
    amount: amount,
    satoshis: amount * 1e8,
    address: address.address,
...
blockchainExplorerMock.broadcast = (raw, cb) => {
  ...
  let hash;
  try {
    let tx = new Ltzcore.Transaction(raw);
...
describe('client API', function() {
  ...
  describe('Client Internals', () => {
    it('should expose ltzcore', () => {
      should.exist(Ltzcore);
      should.exist(Ltzcore.HDPublicKey);
    });
  });
  // todo
  ...
  describe('Build & sign txs', () => {
    ...
    var derivedPrivateKey = {
      BIP44: new Ltzcore.HDPrivateKey(masterPrivateKey).deriveChild("m/44'/1'/0'").toString(),
      BIP45: new Ltzcore.HDPrivateKey(masterPrivateKey).deriveChild("m/45'").toString(),
      BIP48: new Ltzcore.HDPrivateKey(masterPrivateKey).deriveChild("m/48'/1'/0'").toString()
    };
    ...
    describe('#buildTx', () => {
      it('Raw tx roundtrip', () => {
        ...
        var t2 = new Ltzcore.Transaction(t);
      ...
      it('should build a tx correctly (BIP44)', () => {
        ...
        var ltzcoreError = t.getSerializationError({
          disableIsFullySigned: true,
          disableSmallFees: true,
          disableLargeFees: true
        });

        should.not.exist(ltzcoreError);
      ...
      it('should build a P2WSH tx correctly (BIP48)', () => {
        var publicKeyRing = [
          {
            xPubKey: new Ltzcore.HDPublicKey(derivedPrivateKey['BIP48'])
          }
        ];
      ...
      it('should build an eth txp correctly', () => {
        ...
        const publicKeyRing = [
          {
            xPubKey: new Ltzcore.HDPrivateKey(masterPrivateKey).deriveChild(path).toString()
          }
        ];
      ...
      it('should build a tx with provided output scripts', () => {
        ...
        var changeScript = Ltzcore.Script.fromAddress(txp.changeAddress.address).toHex();
    ...
    describe('#pushSignatures', () => {
      it('should sign BIP45 P2SH correctly', () => {
        ...
        var publicKeyRing = [
          {
            xPubKey: new Ltzcore.HDPublicKey(derivedPrivateKey['BIP45'])
          }
        ];
        ...
        // This is a GOOD test, since ltzcore ONLY accept VALID signatures
  ...
  describe('Wallet Creation', () => {
    ...
    it('should be able to access wallet name in non-encrypted wallet (legacy)', done => {
      ...
      var wpk = new Ltzcore.PrivateKey();
      var args = {
        name: 'mywallet',
        m: 1,
        n: 1,
      ...
      helpers.createAndJoinWallet(clients, keys, 2, 3, {}, () => {
        helpers.tamperResponse(
          ...
          status => {
            ...
            status.wallet.copayers[1].xPubKey = new Ltzcore.HDPrivateKey().publicKey;
          ...
          clients[0].pushSignatures(txps[0], signatures, (err, xx, paypro) => {
            ...
            clients[1].pushSignatures(xx, signatures, (err, yy, paypro) => {
              ...
              clients[1].broadcastTxProposal(yy, (err, zz, memo) => {
                ...
                var tx = new Ltzcore.Transaction(rawTx);
          ...
          clients[0].pushSignatures(x2, signatures, (err, txp) => {
            ...
            if (doBroadcast) {
              clients[0].broadcastTxProposal(txp, (err, txp) => {
                ...
                txp.txid.should.equal(new Ltzcore.Transaction(blockchainExplorerMock.lastBroadcasted).id);
                done();
              });
            } else {
              done();
    ...
    describe('1-1 wallets', () => {
      ...
      it('should add access with copayer name', done => {
        ...
        clients[0].addAccess(opts2, (err, x, key) => {
          ...
          var k = new Ltzcore.PrivateKey(key);
          var c = clients[0].credentials;
          c.requestPrivKey = k.toString();
          c.requestPubKey = k.toPublicKey().toString();
