...
The main use that we expect you'll have for the `URI` class in ltzcore is validating and parsing bitcoin URIs. A `URI` instance exposes the address as a ltzcore `Address` object and the amount in Satoshis, if present.
