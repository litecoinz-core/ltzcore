...
async.series([
  function(next) {
    ...
    for (var i = 0; i < 100; i++) {
      ...
      // uint64le
      var br = new ltzcore.encoding.BufferWriter();
      var num = Math.round(Math.random() * 10000000000000);
      br.writeUInt64LEBN(new ltzcore.crypto.BN(num));
      buffers.push(br.toBuffer());

      // hashes
      var data = ltzcore.crypto.Hash.sha256sha256(new Buffer(32));
    ...
    function readUInt64LEBN() {
      ...
      var buf = buffers[c];
      var br = new ltzcore.encoding.BufferReader(buf);
  ...
  function(next) {
    ...
    function ltzcoreTest() {
      block1 = ltzcore.Block.fromString(blockData);
    }
    ...
    var suite = new benchmark.Suite();
    suite.add('ltzcore', ltzcoreTest, {maxTime: maxTime});
