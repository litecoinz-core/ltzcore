...
describe('Messages', function() {
  ...
  describe('@constructor', function() {
    it('sets properties correctly', function() {
      var network = ltzcore.Networks.defaultNetwork;
      var messages = new Messages({
        network: network,
        Block: ltzcore.Block,
        Transaction: ltzcore.Transaction
      });
      should.exist(messages.builder.commands);
      should.exist(messages.builder.constructors);
      messages.builder.constructors.Block.should.equal(ltzcore.Block);
      messages.builder.constructors.Transaction.should.equal(ltzcore.Transaction);
      messages.network.should.deep.equal(network);
    });
    it('network should be unique for each set of messages', function() {
      var messages = new Messages({
        network: ltzcore.Networks.livenet
      });
      var messages2 = new Messages({
        network: ltzcore.Networks.testnet
      });
      messages.network.should.deep.equal(ltzcore.Networks.livenet);
      messages2.network.should.deep.equal(ltzcore.Networks.testnet);
      var message1 = messages.Version();
      message1.network.should.deep.equal(ltzcore.Networks.livenet);
      var message2 = messages2.Version();
      message2.network.should.deep.equal(ltzcore.Networks.testnet);
    });
  });
  ...
  describe('Default Network', function() {
    ...
    Object.keys(messages.builder.commandsMap).forEach(function(command) {
      ...
      it(name, function() {
        var message = messages[name]();
        message.network.should.deep.equal(ltzcore.Networks.defaultNetwork);
      });
    });
