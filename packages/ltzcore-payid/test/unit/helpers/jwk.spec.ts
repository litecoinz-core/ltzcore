...
describe('JsonWebKey', () => {
  ...
  describe('getThumbprint', () => {
    ...
    if (!utils.inBrowser()) { // Compare to other JWK lib thumbprints
      ...
      let rsaJWK;
      before(() => {
        const ltzcoreHD = Ltzcore.HDPrivateKey.fromString(TestKeys.LtzcoreHD);
        const ltzcore = Ltzcore.PrivateKey.fromString(TestKeys.Ltzcore);
        ltzcoreHdJWK = toKey({
          kty: 'EC',
          crv: 'secp256k1',
          x: ltzcoreHD.publicKey.point.x.toBuffer().toString('base64'),
          y: ltzcoreHD.publicKey.point.y.toBuffer().toString('base64')
        });
        ltzcoreJWK = toKey({
          kty: 'EC',
          crv: 'secp256k1',
          x: ltzcore.publicKey.point.x.toBuffer().toString('base64'),
          y: ltzcore.publicKey.point.y.toBuffer().toString('base64')
        });
        ecJWK = toKey(TestKeys.EC.privateKey);
        eddsaJWK = toKey(TestKeys.ED25519.privateKey);
        rsaJWK = toKey(TestKeys.RSA.privateKey);
      });
      it('should be equal for LtzcoreHD key', () => {
        const jwk = new JsonWebKey(ltzcoreHdJWK, ltzcoreHdJWK.type);
        const hex = jwk.getThumbprint('base64');
        hex.should.equal(ltzcoreHdJWK.thumbprint);
      });
      it('should be equal for Ltzcore key', () => {
        const jwk = new JsonWebKey(ltzcoreJWK, ltzcoreJWK.type);
        const hex = jwk.getThumbprint('base64');
        hex.should.equal(ltzcoreJWK.thumbprint);
      });
