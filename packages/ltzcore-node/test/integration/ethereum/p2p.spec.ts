import * as LtzcoreClient from 'ltzcore-client';
...
const { StreamUtil } = LtzcoreClient;
const chain = 'ETH';
...
async function getWallet() {
  let wallet: LtzcoreClient.Wallet;
  try {
    wallet = await LtzcoreClient.Wallet.loadWallet({ name });
    await wallet.register();
    await wallet.syncAddresses();
    return wallet;
  } catch (e) {
    console.log('Creating a new ethereum wallet');
    wallet = await LtzcoreClient.Wallet.create({
      name,
      chain,
      network,
      baseUrl,
