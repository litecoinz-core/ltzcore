...
describe('Ripple Api', function() {
  ...
  it('should transform a ripple rpc response into a ltzcore transaction', async () => {
    const txs = (RippleTxs as any) as Array<FormattedTransactionType>;
    for (const tx of txs) {
      const ltzcoreTx = (await XRP.transform(tx, 'testnet')) as IXrpTransaction;
      expect(ltzcoreTx).to.have.property('chain');
      expect(tx.address).to.eq(ltzcoreTx.from);
      expect(tx.outcome.ledgerVersion).to.eq(ltzcoreTx.blockHeight);
      expect(tx.outcome.fee).to.eq((ltzcoreTx.fee / 1e6).toString());
      expect(Number(tx.outcome.balanceChanges[ltzcoreTx.from][0].value)).to.be.lt(0);
      if (tx.outcome.deliveredAmount) {
        expect(Object.keys(tx.outcome.balanceChanges)).to.contain(ltzcoreTx.to!);
        expect(tx.outcome.deliveredAmount!.value).to.eq((ltzcoreTx.value / 1e6).toString());
        expect(Number(tx.outcome.balanceChanges[ltzcoreTx.to!][0].value)).to.be.gt(0);
      }
    }
  });
  ...
  it('should tag txs from a wallet', async () => {
    ...
    for (const tx of txs) {
      const ltzcoreTx = (await XRP.transform(tx, network)) as IXrpTransaction;
      const ltzcoreCoins = XRP.transformToCoins(tx, network);
      const { transaction, coins } = await XRP.tag(chain, network, ltzcoreTx, ltzcoreCoins);
  ...
  it('should save tagged transactions to the database', async () => {
    ...
    for (const tx of txs) {
      const ltzcoreTx = XRP.transform(tx, network) as IXrpTransaction;
      const ltzcoreCoins = XRP.transformToCoins(tx, network);
      const { transaction, coins } = await XRP.tag(chain, network, ltzcoreTx, ltzcoreCoins);
