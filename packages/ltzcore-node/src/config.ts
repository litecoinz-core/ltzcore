...
function findConfig(): ConfigType | undefined {
  ...
  if (overrideConfig) {
    ltzcoreConfigPaths.unshift(overrideConfig);
  }
  // No config specified. Search home, ltzcore and cur directory
  for (let path of ltzcoreConfigPaths) {
    if (!foundConfig) {
      try {
        const expanded = path[0] === '~' ? path.replace('~', homedir()) : path;
        const ltzcoreConfig = require(expanded) as { ltzcoreNode: ConfigType };
        foundConfig = ltzcoreConfig.ltzcoreNode;
      } catch (e) {
