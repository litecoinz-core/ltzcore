...
export class BitcoinP2PWorker extends BaseP2PWorker<IBtcBlock> {
  protected ltzcoreLib: any;
  protected ltzcoreP2p: any;
  ...
  constructor({ chain, network, chainConfig, blockModel = BitcoinBlockStorage }) {
    ...
    this.ltzcoreLib = Libs.get(chain).lib;
    this.ltzcoreP2p = Libs.get(chain).p2p;
    ...
    this.invCache = {};
    this.invCacheLimits = {
      [this.ltzcoreP2p.Inventory.TYPE.BLOCK]: 100,
      [this.ltzcoreP2p.Inventory.TYPE.TX]: 100000
    };
    this.messages = new this.ltzcoreP2p.Messages({
      network: this.ltzcoreLib.Networks.get(this.network)
    });
    this.pool = new this.ltzcoreP2p.Pool({
  ...
  setupListeners() {
    ...
    this.pool.on('peertx', async (peer, message) => {
      ...
      if (this.isSyncingNode && !this.isCachedInv(this.ltzcoreP2p.Inventory.TYPE.TX, hash)) {
        this.cacheInv(this.ltzcoreP2p.Inventory.TYPE.TX, hash);
    ...
    this.pool.on('peerblock', async (peer, message) => {
      ...
      const blockInCache = this.isCachedInv(this.ltzcoreP2p.Inventory.TYPE.BLOCK, hash);
      if (!blockInCache) {
        block.transactions.forEach(transaction => this.cacheInv(this.ltzcoreP2p.Inventory.TYPE.TX, transaction.hash));
        this.cacheInv(this.ltzcoreP2p.Inventory.TYPE.BLOCK, hash);
      }
    ...
    this.pool.on('peerinv', (peer, message) => {
      if (this.isSyncingNode) {
        const filtered = message.inventory.filter(inv => {
          const hash = this.ltzcoreLib.encoding
