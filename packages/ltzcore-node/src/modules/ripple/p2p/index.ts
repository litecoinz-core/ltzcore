          ...
          .pipe(
            new Transform({
              ...
              transform: async (data, _, cb) => {
                ...
                for (const tx of txs) {
                  const ltzcoreTx = this.provider.transform(tx, network) as IXrpTransaction;
                  const ltzcoreCoins = this.provider.transformToCoins(tx, network);
                  const { transaction, coins } = await this.provider.tag(chain, network, ltzcoreTx, ltzcoreCoins);
