...
See [ltzcore-wallet-client](https://github.com/bitpay/ltzcore/tree/master/packages/ltzcore-wallet-client) for the _official_ client library that communicates to BWS and verifies its response. Also check [ltzcore-wallet](https://github.com/bitpay/ltzcore/tree/master/packages/ltzcore-wallet) for a simple CLI wallet implementation that relies on BWS.
...
BWS supports SSL and Clustering. For a detailed guide on installing BWS with extra features see [Installing BWS](https://github.com/bitpay/ltzcore/blob/master/packages/ltzcore-wallet-service/installation.md).
...
- A copayer could join the wallet more than once, and there is no mechanism to prevent this. See [wallet](https://github.com/bitpay/ltzcore/tree/master/packages/ltzcore-wallet)'s confirm command, for a method for confirming copayers.
...
- ltzcore-node running on http://localhost:3000
...
> **ltzcore-wallet-service/lib/notificationbroadcaster.js**
...
1.  Add regtest to ltzcore.config.json.
...
**ltzcore-wallet-service/config.js**
...
See [Ltzcore Wallet Client](https://github.com/bitpay/ltzcore/tree/master/packages/ltzcore-wallet-client) for implementation details.
...
- Wallet object. (see [fields on the source code](https://github.com/bitpay/ltzcore/blob/master/packages/ltzcore-wallet-service/src/lib/model/wallet.ts)).
...
- TX Proposal object. (see [fields on the source code]https://github.com/bitpay/ltzcore/blob/master/packages/ltzcore-wallet-service/src/lib/model/txproposal.ts)). `.id` is probably needed in this case.

### `/v2/txproposals/:id/publish`: Publish the previously created `temporary` tx proposal

Returns:

- TX Proposal object. (see [fields on the source code](https://github.com/bitpay/ltzcore/blob/master/packages/ltzcore-wallet-service/src/lib/model/txproposal.ts)).

### `/v3/addresses/`: Request a new main address from wallet . (creates an address on normal conditions)

Returns:

- Address object: (https://github.com/bitpay/ltzcore/blob/master/packages/ltzcore-wallet-service/src/lib/model/address.ts). Note that `path` is returned so client can derive the address independently and check server's response.
...
- TX Proposal object. (see [fields on the source code](https://github.com/bitpay/ltzcore/blob/master/packages/ltzcore-wallet-service/src/lib/model/txproposal.ts)). `.status` is probably needed in this case.
