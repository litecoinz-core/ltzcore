...
helpers.signMessage = function(message, privKey) {
  var priv = new Ltzcore.PrivateKey(privKey);
  const flattenedMessage = _.isArray(message)? _.join(message) : message;
  var hash = Utils.hashMessage(flattenedMessage);
  return Ltzcore.crypto.ECDSA.sign(hash, priv, 'little').toString();
};

helpers.signRequestPubKey = function(requestPubKey, xPrivKey) {
  var priv = new Ltzcore.HDPrivateKey(xPrivKey).deriveChild(Constants.PATHS.REQUEST_KEY_AUTH).privateKey;
...
helpers._generateCopayersTestData = function() {
  ...
  _.each(xPrivKeys, function(xPrivKeyStr, c) {
    var xpriv = Ltzcore.HDPrivateKey(xPrivKeyStr);
    var xpub = Ltzcore.HDPublicKey(xpriv);

    var xpriv_45H = xpriv.deriveChild(45, true);
    var xpub_45H = Ltzcore.HDPublicKey(xpriv_45H);
...
helpers.randomTXID = function() {
  return Ltzcore.crypto.Hash.sha256(Buffer.from((Math.random() * 100000).toString())).toString('hex');;
};
...
helpers.stubUtxos = function(server, wallet, amounts, opts, cb) {
  ...
  var S = Ltzcore_[wallet.coin].Script;
  async.waterfall([
