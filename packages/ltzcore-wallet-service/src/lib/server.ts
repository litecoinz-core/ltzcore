...
export class WalletService {
  ...
  createWallet(opts, cb) {
    ...
    try {
      pubKey = new Ltzcore.PublicKey.fromString(opts.pubKey);
    } catch (ex) {
  ...
  _verifyRequestPubKey(requestPubKey, signature, xPubKey) {
    const pub = new Ltzcore.HDPublicKey(xPubKey).deriveChild(Constants.PATHS.REQUEST_KEY_AUTH).publicKey;
  ...
  joinWallet(opts, cb) {
    ...
    let xPubKey;
    try {
      xPubKey = Ltzcore.HDPublicKey(opts.xPubKey);
    } catch (ex) {
      ...
      [
        ...
        next => {
          ...
          this._getBlockchainHeight(wallet.coin, wallet.network, (err, height, hash) => {
            ...
            const dustThreshold = Ltzcore_[wallet.coin].Transaction.DUST_AMOUNT;
  ...
  getUtxos(opts, cb) {
    ...
    if (opts.addresses) {
      ...
      this.getWallet({}, (err, wallet) => {
        ...
        const A = Ltzcore_[wallet.coin].Address;
        ...
        (err, txp) => {
          ...
          try {
            if (!txp.sign(this.copayerId, opts.signatures, copayer.xPubKey)) {
              ...
              const raw = ChainService.getLtzcoreTx(txp).uncheckedSerialize();
    ...
    this.storage.getWalletAddressChecked(wallet.id, (err, checkedTotal) => {
      ...
      this.storage.walletCheck({ walletId: wallet.id }).then((localCheck: { sum: number }) => {
        bc.getCheckData(wallet, (err, serverCheck) => {
          ...
          if (err) {
            this.logw('Error at ltzcore WalletCheck, ignoring' + err);
            return cb();
          }
  ...
  wyreWalletOrderQuotation(req): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      const XApiSignatureHash: string = Ltzcore.crypto.Hash.sha256hmac(
