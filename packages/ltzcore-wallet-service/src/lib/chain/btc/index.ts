...
export class BtcChain implements IChain {
  ...
  constructor(private ltzcoreLib = LtzcoreLib) {
  ...
  getDustAmountValue() {
    return this.ltzcoreLib.Transaction.DUST_AMOUNT;
  }
  ...
  checkDust(output) {
    const dustThreshold = Math.max(Defaults.MIN_OUTPUT_AMOUNT, this.ltzcoreLib.Transaction.DUST_AMOUNT);
  ...
  getEstimatedSizeForSingleOutput(address?: string) {
    ...
    if (address) {
      const a = this.ltzcoreLib.Address(address);
      addressType = a.type;
    }
  ...
  getLtzcoreTx(txp, opts = { signed: true }) {
    const t = new this.ltzcoreLib.Transaction();
    ...
    switch (txp.addressType) {
      ...
      case Constants.SCRIPT_TYPES.P2SH:
        _.each(inputs, i => {
          $.checkState(i.publicKeys, 'Failed state: Inputs should include public keys at <getLtzcoreTx()>');
    ...
    _.each(txp.outputs, o => {
      $.checkState(
        ...
        'Failed state: Output should have either toAddress or script specified at <getLtzcoreTx()>'
      );
      if (o.script) {
        t.addOutput(
          new this.ltzcoreLib.Transaction.Output({
            script: o.script,
            satoshis: o.amount
          })
        );
    ...
    if (t.outputs.length > 1) {
      ...
      $.checkState(
        ...
        'Failed state: t.outputs.length not equal to outputOrder.length at <getLtzcoreTx()>'
      );
    ...
    $.checkState(
      ...
      'Failed state: not-enough-inputs at <getLtzcoreTx()>'
    );
    $.checkState(
      totalInputs - totalOutputs <= Defaults.MAX_TX_FEE[txp.coin],
      'Failed state: fee-too-high at <getLtzcoreTx()>'
    );

    if (opts.signed) {
      const sigs = txp.getCurrentSignatures();
      _.each(sigs, x => {
        this.addSignaturesToLtzcoreTx(t, txp.inputs, txp.inputPaths, x.signatures, x.xpub, txp.signingMethod);
      });
    }
    return t;
  }
  ...
  checkTx(txp) {
    ...
    try {
      const ltzcoreTx = this.getLtzcoreTx(txp);
      ltzcoreError = ltzcoreTx.getSerializationError(serializationOpts);
      if (!ltzcoreError) {
        txp.fee = ltzcoreTx.getFee();
      }
    } catch (ex) {
      logger.warn('Error building Ltzcore transaction', ex);
      return ex;
    }

    if (ltzcoreError instanceof this.ltzcoreLib.errors.Transaction.FeeError)
      return new ClientError(
        Errors.codes.INSUFFICIENT_FUNDS_FOR_FEE,
        Errors.INSUFFICIENT_FUNDS_FOR_FEE.message + ' + coin: ' + txp.coin + ' feePerKb: ' + txp.feePerKb
      );
    if (ltzcoreError instanceof this.ltzcoreLib.errors.Transaction.DustOutputs) return Errors.DUST_AMOUNT;
    return ltzcoreError;
  }
  ...
  addSignaturesToLtzcoreTx(tx, inputs, inputPaths, signatures, xpub, signingMethod) {
    signingMethod = signingMethod || 'ecdsa';
    if (signatures.length != inputs.length) throw new Error('Number of signatures does not match number of inputs');

    let i = 0;
    const x = new this.ltzcoreLib.HDPublicKey(xpub);

    _.each(signatures, signatureHex => {
      try {
        const signature = this.ltzcoreLib.crypto.Signature.fromString(signatureHex);
        const pub = x.deriveChild(inputPaths[i]).publicKey;
        const s = {
          inputIndex: i,
          signature,
          sigtype: this.ltzcoreLib.crypto.Signature.SIGHASH_ALL | this.ltzcoreLib.crypto.Signature.SIGHASH_FORKID,
          publicKey: pub
        };
