...
export class EthChain implements IChain {
  /**
   * Converts Ltzcore Balance Response.
   * @param {Object} ltzcoreBalance - { unconfirmed, confirmed, balance }
   * @param {Number} locked - Sum of txp.amount
   * @returns {Object} balance - Total amount & locked amount.
   */
  private convertLtzcoreBalance(ltzcoreBalance, locked) {
    const { unconfirmed, confirmed, balance } = ltzcoreBalance;
  ...
  getWalletBalance(server, wallet, opts, cb) {
    ...
    bc.getBalance(wallet, (err, balance) => {
      ...
      server.getPendingTxs(opts, (err, txps) => {
        ...
        const convertedBalance = this.convertLtzcoreBalance(balance, lockedSum);
  ...
  getLtzcoreTx(txp, opts = { signed: true }) {
    ...
    if (opts.signed) {
      ...
      sigs.forEach(x => {
        this.addSignaturesToLtzcoreTx(tx, txp.inputs, txp.inputPaths, x.signatures, x.xpub);
      });
    }
  ...
  checkTx(txp) {
    try {
      const tx = this.getLtzcoreTx(txp);
    } catch (ex) {
      logger.debug('Error building Ltzcore transaction', ex);
      return ex;
    }
  ...
  addSignaturesToLtzcoreTx(tx, inputs, inputPaths, signatures, xpub) {
